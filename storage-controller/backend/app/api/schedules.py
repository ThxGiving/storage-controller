...
def _sched_out(s: ReportSchedule) -> ScheduleOut:
    ...
    return ScheduleOut(
        ...
        next_run_utc=ensure_utc(s.next_run_utc)
        or next_run_utc(s.run_day, s.run_time, s.timezone, datetime.now(UTC)),
        last_run_utc=ensure_utc(s.last_run_utc), last_result=s.last_result,
...
def _delivery_out(d: EmailDelivery | None) -> EmailDeliveryOut | None:
    ...
    return EmailDeliveryOut(
        ...
        next_attempt_utc=ensure_utc(d.next_attempt_utc), last_error_category=d.last_error_category,
...
async def _run_out(db: AsyncSession, run: ScheduleRun) -> ScheduleRunOut:
    ...
    return ScheduleRunOut(
        ...
        scheduled_for_utc=ensure_utc(run.scheduled_for_utc), state=run.state, trigger=run.trigger,
        ...
        started_at=ensure_utc(run.started_at), finished_at=ensure_utc(run.finished_at),
