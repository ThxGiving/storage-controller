...
from ..history_import import RECOMMENDED_RANGE, check_availability, run_import
from ..models import (
    EntityAssignment,
    EntityRole,
    HistoryImport,
...
from ..schemas import HistoryAvailabilityOut, HistoryImportOut, HistoryImportStart
...
async def start_import(
    ...
    # Don't start a second concurrent import for the same unit.
...
async def _run(job_id, assignment_id, unit_id, rest, ws_url, token, entity_unit, tz_name) -> None:
    if rest is None:
        return
    ...
    async with factory() as session:
        ...
        try:
            await run_import(
                ...
                rest=rest, ws_url=ws_url, token=token, entity_unit=entity_unit, tz_name=tz_name,
            )
